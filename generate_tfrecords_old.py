...
def create_tf_example(group, path, category_idx):
    ...
    for index, row in group.object.iterrows():
        xmins.append(float(row['x_min']) / width)
        xmaxs.append(float(row['x_max']) / width)
        ymins.append(float(row['y_min']) / height)
        ymaxs.append(float(row['y_max']) / height)
